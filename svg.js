...
;(function() {
  ...
  SVG.create = function(name) {
    /* create element */
    var element = SVG.document.createElementNS(this.ns, name)
  ...
  SVG.get = function(id) {
    var node = SVG.document.getElementById(id)
    if (node) return node.instance
  }
  
  // svg support test
  SVG.supported = true;//(function() {
  //  return !! SVG.document.createElementNS &&
  //         !! SVG.document.createElementNS(SVG.ns,'svg').createSVGRect
  //})()
  ...
  SVG.RBox = function(element) {
    ...
    if (element) {
      ...
      /* calculate cumulative zoom from svg SVG.documents */
      e = element
      while (e = e.parent) {
  ...
  SVG.Doc = function(element) {
    ...
    this.parent = typeof element == 'string' ?
      SVG.document.getElementById(element) :
      element
    
    /* If the target is an svg element, use that element as the main wrapper.
       This allows svg.js to work with svg SVG.documents as well. */
    this.constructor
  ...
  SVG.extend(SVG.Doc, {
    ...
    stage: function() {
      var check
        , element = this
        , wrapper = SVG.document.createElement('div')
      ...
      /* check for dom:ready */
      check = function() {
        if (SVG.document.readyState === 'complete') {
  ...
  SVG.extend(SVG.TSpan, {
    // Set text content
    text: function(text) {
      this.node.appendChild(SVG.document.createTextNode(text))
  ...
  //
  SVG.extend(SVG.Container, {
    // Create nested svg SVG.document
    nested: function() {
